<template>
  <div>
    <v-toolbar flat color="primary">
      <v-toolbar-title>Room [{{this.room_id}}] History</v-toolbar-title>
      <v-divider
        class="mx-2"
        inset
        vertical
      ...
      <v-btn fab color="white">
       <v-icon dark color="primary" @click="getRoomHistory()">refresh</v-icon>
  </v-btn>
    </v-toolbar>
    <v-data-table
      :headers="headers"
...
import { BarChart } from 'vue-morris'
  export default {
    ...
    methods: {
      getRoomHistory () {
        axios.get('api/roomHistory/'+this.room_id).then(response=>{
          this.roomHistory=response.data
        }).catch(error=>{
          this.roomHistory=[]
        })
